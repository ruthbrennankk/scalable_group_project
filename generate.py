...
def main():
    ...
    parser.add_argument('--output-labels', help='Where to store the generated captchas label file', type=str)
    ...
    if args.output_labels is None:
        print("Please specify the captcha output label file")
        exit(1)
    ...
    for i in range(args.count):
        ...
        if os.path.exists(image_path):
            version = 1
            while os.path.exists(os.path.join(args.output_dir, img_name + '_' + str(version) + '.png')):
                version += 1
            image_path = os.path.join(args.output_dir, img_name + '_' + str(version) + '.png')
